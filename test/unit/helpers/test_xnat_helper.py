...
class TestXNATHelper(object):
    ...
    def test_find_experiment(self):
        with xnat_helper.connection() as xnat:
            ...
            assert_true(sbj.exists(), "Subject %s session not created: %s" % (SUBJECT, SESSION))
            sbj = xnat.find(project(), SUBJECT, SESSION, create=True)
            assert_true(sbj.exists(), "Subject %s session not found: %s" % (SUBJECT, SESSION))

    def test_find_resource(self):
        with xnat_helper.connection() as xnat:
            sbj = xnat.find(project(), SUBJECT, SESSION, resource=REGISTRATION, create=True)
    ...
    def test_find_reconstruction(self):
        with xnat_helper.connection() as xnat:
            sbj = xnat.find(project(), SUBJECT, SESSION, reconstruction=RECONSTRUCTION, create=True)
            assert_true(sbj.exists(),
                        "Subject %s session %s reconstruction not created: %s" %
                        (SUBJECT, SESSION, RECONSTRUCTION))
            sbj = xnat.find(project(), SUBJECT, SESSION, reconstruction=RECONSTRUCTION, create=True)
            assert_true(sbj.exists(),
                        "Subject %s session %s reconstruction not found: %s" %
                        (SUBJECT, SESSION, RECONSTRUCTION))

    def test_find_assessor(self):
        with xnat_helper.connection() as xnat:
            sbj = xnat.find(project(), SUBJECT, SESSION, assessor=ASSESSOR, create=True)
            assert_true(sbj.exists(),
    ...
    def test_registration_round_trip(self):
        with xnat_helper.connection() as xnat:
            ...
            scan_obj = xnat.get_scan(project(), SUBJECT, SESSION, SCAN)
            assert_true(scan_obj.exists(),
                        "XNAT %s %s %s %s resource does not exist." %
                        (project(), SUBJECT, SESSION, SCAN))
            file_obj = scan_obj.resource('NIFTI').file(fname)
            assert_true(file_obj.exists(), "File not uploaded: %s" % fname)
