...
class TestXNATUpload(object):
    ...
    def test_upload_scan(self):
        logger(
            __name__).debug("Testing the XNATUpload interface on %s %s scan %d..." %
        ...
        with xnat_helper.connection() as xnat:
            ...
            assert_true(
                scan_obj.exists(), "Upload did not create the %s %s scan: %s" %
                (SUBJECT, SESSION, SCAN))
    ...
    def test_upload_reconstruction(self):
        logger(
            __name__).debug("Testing the XNATUpload interface on %s %s reconstruction %s..." %
        ...
        # Verify the result.
        with xnat_helper.connection() as xnat:
            recon_obj = xnat.get_reconstruction(
                project(), SUBJECT, SESSION, RECON)
    ...
    def test_upload_analysis(self):
        logger(
            __name__).debug("Testing the XNATUpload interface on %s %s analysis %s..." %
        ...
        with xnat_helper.connection() as xnat:
            ...
            assert_true(
                anl_obj.exists(), "Upload did not create the %s %s analysis: %s" %
                (SUBJECT, SESSION, ANALYSIS))
