...
class Unpack(IOBase):
    """
    The Unpack Interface converts a list input field to one output
    field per list item.
    ...
    def __init__(self, input_name, output_names, mandatory_inputs=True,
                 **inputs):
        """
        :param input_name: the input list field name
        :param output_names: the output field names
        :param mandatory_inputs: a flag indicating whether every input
            field is required
        ...
        if not output_names:
            raise InterfaceError('Unpack output field names must be a ' +
                                 'non-empty list')
        ...
        # initialization, even it the trait is not in the add_traits
        # argument. The work-around is to reset the values after adding
        # the traits.
    ...
    def _run_interface(self, runtime):
            ...
            for key in self.input_names:
                ...
                if not isdefined(value):
                    msg = ("%s requires a value for input '%s' because"
                        " it was listed in 'input_names'. You can turn"
                        " off mandatory inputs checking by passing"
                        " mandatory_inputs=False to the constructor." %
        ...
        self._result = {self.output_names[i]: value
