...
# Match the subject, session and series names from the input series directory.
pvs = pe.Node(interface=DataFinder(max_depth=0, unpack_single=True), name='pvs')
pvs.inputs.match_regex='.+/(?P<subject>subject\d{2})/(?P<session>session\d{2})/(?P<series>series\d{3})'
